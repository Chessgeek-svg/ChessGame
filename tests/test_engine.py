...
class TestGameState:
    ...
    def test_castling_rights_logic(self):
        ...
        assert (
            game.castling_rights["White"]["kingside"] is False
        ), "Castling rights not lost after rook move"

        game.undo_move()
        assert (
            game.castling_rights["White"]["kingside"] is True
        ), "Castling rights not restored after undo"

        move = Move((0, 4), (1, 4), game.board)
        game.make_move(move)
        assert (
            game.castling_rights[move.piece_moved.color]["kingside"] is False
        ), "Castling rights not lost after king move"
        assert (
            game.castling_rights[move.piece_moved.color]["queenside"] is False
        ), "Castling rights not lost after king move"
        ...
        game.undo_move()
        assert (
            game.castling_rights[move.piece_moved.color]["kingside"] is True
        ), "Castling rights not restored after undo"
        assert (
            game.castling_rights[move.piece_moved.color]["queenside"] is True
        ), "Castling rights not restored after undo"
        assert game.castling_rights_log == []
    ...
    def test_en_passant_logic(self):
        ...
        game.make_move(move)
        assert game.en_passant_target == (
            5,
            4,
        ), "En Passant Target square not properly generated"
        assert game.en_passant_target_log == [
            None
        ], "En Passant Target log not updating correctly"

        en_passant = Move((4, 3), (5, 4), game.board, is_en_passant=True)
        game.make_move(en_passant)
        assert (
            game.en_passant_target == None
        ), "En Passant Target square not properly cleared"
        assert game.en_passant_target_log == [
            None,
            (5, 4),
        ], "En Passant Target log not updating correctly"
        assert (
            white_pawn.square == None
        ), "Captured piece's square not updating correctly"
        assert black_pawn.square == (
            5,
            4,
        ), "Capturing piece's square not updating correctly"
        assert (
            game.board[4][4] == None
        ), "Board does not recognized that piece has been captured"

        game.undo_move()
        assert game.en_passant_target == (
            5,
            4,
        ), "En Passant Target square not properly restored"
        assert game.en_passant_target_log == [
            None
        ], "En Passant Target log not restoring correctly"
        assert white_pawn.square == (
            4,
            4,
        ), "Captured piece's square not restoring correctly"
        assert black_pawn.square == (
            4,
            3,
        ), "Capturing piece's square not restoring correctly"
        assert (
            game.board[4][4] == white_pawn
        ), "Board does not recognized that piece has been un-captured"
    ...
    def test_promotion_and_undo(self):
        ...
        assert isinstance(
            empty_game.white_pieces[1], Queen
        ), "Promoted piece not generated"
        assert pawn.square == None, "Pawn not removed from board after promoting"
        assert (
            empty_game.board[1][0] == None
        ), "Pawn not removed from board after promoting"

        empty_game.undo_move()

        assert isinstance(
            empty_game.board[1][0], Pawn
        ), "Pawn not restored to board after undo"
        assert (
            len(empty_game.white_pieces) == 1
        ), "Promoted piece not removed after undo"
        assert pawn.square == (1, 0), "Pawn not restored to board after un-promoting"
        assert (
            empty_game.board[1][0] == pawn
        ), "Pawn not restored to board after un-promoting"
    ...
    def test_castling_out_of_through_check(self):
        ...
        assert (
            len(empty_game.white_pieces) == 2
        ), "Manually placed pieces not added to piece list"
        assert (
            len(empty_game.black_pieces) == 2
        ), "Manually placed piece not added to piece list"

        assert (
            empty_game.square_under_attack((7, 5)) is True
        ), "Square not under attack when it should be"
        assert (
            empty_game.square_under_attack((7, 4)) is False
        ), "Square under attack when it shouldn't be"
        ...
        assert (
            len(empty_game.black_pieces) == 1
        ), "Manually removed piece not removed from piece list"
