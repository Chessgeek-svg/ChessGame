...
class Gamestate(object):
    ...
    def get_all_legal_moves(self):
        ...
        for move in candidate_moves:
            ...
            if move.is_castle:
                if self.square_under_attack(
                    (move.start_row, move.start_col)
                ) or self.square_under_attack(
                    (move.start_row, (move.start_col + move.end_col) // 2)
                ):
                    continue
    ...
    def update_castle_rights(self, move):
        if isinstance(move.piece_moved, King):
            ...
            self._remove_rook_rights(
                move.piece_moved.color, (move.start_row, move.start_col)
            )
        if isinstance(move.piece_captured, Rook):
            self._remove_rook_rights(
                move.piece_captured.color, (move.end_row, move.end_col)
            )
