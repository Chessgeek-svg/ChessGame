        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if (
                    sq_selected != (row, col)
                    and square
                    and square.color == ("White" if gs.white_to_move else "Black")
                ):
                    sq_selected = (row, col)
                ...
                if len(player_clicks) == 2:
                    ...
                    candidate_moves = [
                        move for move in legal_moves if move == temp_move
                    ]
                    chosen_move = None
...
def draw_move_log(screen, gs, font):
    ...
    move_text_log = []
    moves_to_show = (
        88  # Current hardcoded window and font sizes allow 44 move pairs to render
    )
    ...
    for i in range(start_index, len(move_log), 2):
        ...
        if i + 1 < len(
            move_log
        ):  # If black has an accompanying move, print that as well
...
def highlight_squares(screen, gs, legal_moves, sq_selected):
    ...
    if sq_selected != ():
        ...
        if gs.board[row][col] and gs.board[row][col].color == (
            "White" if gs.white_to_move else "Black"
        ):
